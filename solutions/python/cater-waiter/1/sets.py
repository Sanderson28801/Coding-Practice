...
def categorize_dish(dish_name, dish_ingredients):
    ...
    """
    meal_categories = [ VEGAN, VEGETARIAN, KETO, PALEO, OMNIVORE]
    categories_names = ["VEGAN", "VEGETARIAN", "KETO", "PALEO", "OMNIVORE"]
    for index, category in enumerate(meal_categories):
        if set(dish_ingredients).issubset(category):
            return f"{dish_name}: {categories_names[index]}"
...
def singleton_ingredients(dishes, intersection):
    ...
    singleton_items = set()
    for dish in dishes:
        singleton_items = singleton_items ^ dish
    singleton_items = singleton_items - intersection
